...
def anthropic_get_datetime(
    date_format: str, model: str = "claude-haiku-4-5"
) -> Dict[str, Any]:
    """Uses an Anthropic model to get the current datetime in a specified format via a tool call."""
    ...
    user_message = {
        "role": "user",
        "content": f"what is the current date and time, formatted as {date_format}?",
    }
    ...
    try:
        ...
        tool_input = getattr(tool_use, "input", None) or (
            tool_use.get("input") if isinstance(tool_use, dict) else None
        )
        tool_use_id = getattr(tool_use, "id", None) or (
            tool_use.get("id") if isinstance(tool_use, dict) else None
        )
    except Exception:
        tool_input = None
        tool_use_id = None
...
def main():
    """Streamlit app to demonstrate getting current datetime via local Python function or Anthropic tool call."""
    st.title("Current Datetime — Streamlit Demo")
    st.markdown(
        "Small demo that returns the current datetime in a requested format. Choose `Local` or `Anthropic`."
    )
    ...
    st.markdown("#### Notes")
    st.markdown(
        "- Ensure `CLAUDE_API_KEY` (Anthropic key) is available in environment or in a `.env` file when using Anthropic mode."
    )
